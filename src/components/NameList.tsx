import { NameItem, NameItemProps } from './NameItem';

type NameListProps = {
  names: (NameItemProps & { id: string })[];
  onAdd: () => void;
};

export const NameList = ({ names, onAdd }: NameListProps) => {
  return (
    <div className="flex flex-col gap-2 w-full max-w-4xl p-4">
      {names.length === 0 ? (
        ...
        names.map((item) => <NameItem key={item.id} {...item} />)
      )}
      <button
        onClick={onAdd}
